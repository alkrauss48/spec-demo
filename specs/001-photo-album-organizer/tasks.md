...
- [ ] T017 [P] Build the sign-up page and its Server Action in src/app/sign-up/page.tsx and src/app/sign-up/actions.ts with the same field, error, and heading rules as T016, redirecting to `/` on success (ui-routes.md "/sign-up") (FR-014, FR-017, R10)
- [ ] T018 Add a sign-out control (a `<form>` posting a Server Action that calls `auth.api.signOut` and redirects to `/sign-in`) to the root layout header in src/app/layout.tsx (depends on T025) (FR-014, R10)
...
- [ ] T024 [P] Create client component `SafeImage` (`'use client'`): renders `<img>` with required `width`, `height`, `alt`, optional `loading`; on `onError` replaces it with a neutral placeholder `<div role="img" aria-label={fallbackLabel}>` of the same box size (or `aria-hidden` when `alt=""`) in src/components/ui/SafeImage.tsx (FR-016, FR-017, edge case "Photo fails to load")
...
- [ ] T026 [P] Create client component `ErrorReporter` in src/components/ui/ErrorReporter.tsx: registers `window.onerror` and `unhandledrejection` listeners and exports `reportClientError({ message, stack, route, requestId })`, which `POST`s to `/api/client-errors` with `keepalive: true`, sending the route **pattern** (e.g. `/albums/[date]`), not the concrete URL (Principle V)
...
- [ ] T091 [P] Write the read-latency test in tests/perf/read-latency.spec.ts: using the `perf@example.test` seed user (T042), request `/`, one album page, 200 `thumb` URLs, and 50 `full` URLs; compute p95 from `duration_ms` in `test-results/server.log` on the `library.render`, `album.render`, and `media.served` events (for `media.served`, this is the time until headers are sent) and assert ≤ 300 ms (constitution "API response time", plan.md Constraints)
